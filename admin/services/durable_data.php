...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    $sql = "SELECT * $sql_base
        ORDER BY d.dura_id DESC LIMIT :start, :length";
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $status = '';
        if ($row['dura_status'] == 0) {
            $status = '<span class="badge badge-danger">ระงับ</span>';
        }elseif ($row['dura_status'] == 1) {
            $status = '<span class="badge badge-success">ปกติ</span>';
        }elseif ($row['dura_status'] == 2) {
            $status = '<span class="badge badge-secondary">ส่งซ่อม</span>';
        }elseif ($row['dura_status'] == 3) {
            $status = '<span class="badge badge-warning">ชำรุด</span>';
        }

        $booked = '';
        if ($row['dura_booked'] == 'Y') {
            $booked = 'ถูกยืม';
        } else {
            $booked = '-';
        }
